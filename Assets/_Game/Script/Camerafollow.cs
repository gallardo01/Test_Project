...
public class CameraFollow : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        if (GameManager.IsState(GameState.ShopSkin))
        {
            ...
            TF.position = Vector3.Lerp(TF.position, player.TF.position + offsetInShop, Time.deltaTime * 5f);
            TF.rotation = Quaternion.Euler(15f, 0, 0);
            return;

        }
        TF.position = Vector3.Lerp(TF.position, player.TF.position + offset + new Vector3(0, player.attackRange * 1.5f, -player.attackRange * 1.5f), Time.deltaTime * 5f);
    ...
    {
        Vector3 PlayerPos = Camera.main.WorldToScreenPoint(player.TF.position);
